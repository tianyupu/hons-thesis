...
results than automatic feature selection in all but two cases. Neither
...
in AUC of 0.034 but with 29 features, which is still a fairly large
...
accuracy and AUC of all classifiers under each feature selection method.
The methods that do not select features based upon a specified threshold
(manual and baseline as well as C4.5 wrapper and no feature selection)
are graphed in Figure \ref{fig:tr-nothreshold}, and the methods that select
based upon thresholds (correlation and information gain) are graphed
...
selected by C4.5 decision trees and nearest
